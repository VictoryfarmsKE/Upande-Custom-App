...
class RequisitionForm(BuyingController):
# class MaterialRequest(BuyingController):
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				...
				if actual_so_qty and (flt(so_items[so_no][item]) + already_indented > actual_so_qty):
					frappe.throw(
						_("Requisition Form of maximum {0} can be made for Item {1} against Sales Order {2}").format(
							actual_so_qty - already_indented, item, so_no
						)
					)

	def validate(self):
		super(RequisitionForm, self).validate()
		...
		# Though the creation of Requisition Form from a Production Plan can be rethought to fix this
	...
	def check_modified_date(self):
		mod_db = frappe.db.sql(
			"""select modified from `tabRequisition Form` where name = %s""", self.name
		)
	...
	def get_mr_items_ordered_qty(self, mr_items):
		...
		if doctype and qty_field:
			query = (
				...
				.select(doctype.requisition_form_item, Sum(qty_field))
				.where(
					(doctype.requisition_form == self.name)
					& (doctype.requisition_form_item.isin(mr_items))
					& (doctype.docstatus == 1)
				)
				.groupby(doctype.requisition_form_item)
			)
	...
	def update_completed_qty(self, mr_items=None, update_modified=True):
				...
				if self.material_request_type in ("Material Issue", "Material Transfer", "Customer Provided"):
					...
					if mr_qty_allowance:
						...
						if d.ordered_qty and d.ordered_qty > allowed_qty:
							frappe.throw(
								_(
									"The total Issue / Transfer quantity {0} in Requisition Form {1}  cannot be greater than allowed requested quantity {2} for Item {3}"
						...
						frappe.throw(
							_(
								"The total Issue / Transfer quantity {0} in Requisition Form {1} cannot be greater than requested quantity {2} for Item {3}"
		...
		self._update_percent_field(
			{
				"target_dt": "Requisition Form Item",
	...
	if stock_entry.doctype == "Stock Entry":
		...
		for mr, mr_item_rows in material_request_map.items():
			if mr and mr_item_rows:
				mr_obj = frappe.get_doc("Requisition Form", mr)

				if mr_obj.status in ["Stopped", "Cancelled"]:
					frappe.throw(
						_("{0} {1} is cancelled or stopped").format(_("Requisition Form"), mr),
						frappe.InvalidStatusError,
					)
...
def update_status(name, status):
	material_request = frappe.get_doc("Requisition Form", name)
...
def get_material_requests_based_on_supplier(doctype, txt, searchfield, start, page_len, filters):
	...
	material_requests = frappe.db.sql(
		...
		from `tabRequisition Form` mr, `tabRequisition Form Item` mr_item
...
def get_default_supplier_query(doctype, txt, searchfield, start, page_len, filters):
	doc = frappe.get_doc("Requisition Form", filters.get("doc"))
	item_list = []
	for d in doc.items:
...
def raise_work_orders(material_request):
	mr = frappe.get_doc("Requisition Form", material_request)
	errors = []
	work_orders = []
				...
				wo_order.update(
					{
						...
						"requisition_form": mr.name,
						"requisition_form_item": d.name,
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"Requisition Form Item": {
				...
				"field_map": {"name": "requisition_form_item", "qty": "stock_qty"},
			},
		},
		target_doc,
	)
