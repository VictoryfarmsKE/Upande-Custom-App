...
function processIPDraftPayments(frm, draftPymnts, grand_totals) {
    ...
    draftPymnts.forEach(dp => {
         if (!existingPymnts.has(dp.name)) {
            ...
            newRow.beneficiary_account = dp.party_bank_account.bank_account_no;
            newRow.reference = dp.reference_no;
            newRow.beneficiary_email_id = dp.contact_email;
            newRow.debit_amount = dp.paid_amount; // Assuming 'amount' is a field in the child table
            newRow.swift_code = dp.party_bank_account.custom_swift_code;
