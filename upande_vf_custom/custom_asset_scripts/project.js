...
function manage_create_asset_button(frm) {
    ...
    // Only show the button once the doc is saved and clean —
    // this keeps the normal Save button available while editing/drafting.
    if (frm.doc.project_type === "Capital Work in Progress" && frm.doc.name && !frm.is_dirty()) {
    ...
    // No manual removal needed — add_custom_button is redrawn fresh on every refresh(),
    // so the button simply won't appear when the condition above is false.
}
